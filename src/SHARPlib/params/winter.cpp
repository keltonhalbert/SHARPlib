...
void posneg_temperature(float start, const float pres_arr[],
    ...
    float upper, lower, pe1, h1, te1, tp1, totp, totn, pe2, h2, te2, tp2, tdef1,
        tdef2;
    ...
    if (((interp_pressure(500, pres_arr, temperature_arr, N) < -273.15) &&
         (interp_pressure(850, pres_arr, temperature_arr, N) < -273.15)) ||
        (start < -1))
        return;
    ...
    while (pres_arr[i] < upper) {
        i--;
        if (i < 0) {
            // fprintf(stderr,"Warning: posneg_temp: Could not find a pressure
            // greater than %.2f\n",upper); fprintf(stderr, "Using %.2f as the
            // upper level.\n",pres_arr[0]);
            i = 0;
            break;
        }
    ...
    for (i = uptr; i >= lptr; i--) {
        if (temperature_arr[i] > -273.15) {
            ...
            if (warmlayer > 0) {
                ...
                tote += lyre;
                // printf("%4.0f - %4.0f E=%6.0f TOT=%6.0f Top=%6.0f
                // Bot=%6.0f\n",
        ...
        for (i = N - 1; i > 0; i--) {
            ...
            if (height_arr[i] - height_arr[0] < 5000.0) {
                ...
                if (rh > 80) {
                    p1 = pres_arr[i] + 5000;
                    if (relh(p1,
                             interp_pressure(p1, pres_arr, temperature_arr, N),
                             interp_pressure(p1, pres_arr, dewpoint_arr, N)) >
                        80) {
                        ok = 1;
                        pbegin = p1 - 2500.0;
                        break;
