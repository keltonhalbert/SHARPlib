...
float interp_coord(float data_val, const float data_arr[],
    ...
    for (int k = 0; k < N - 1; ++k) {
        int idx_top = k + 1;
        const float data_bot = data_arr[k];
        const float coord_bot = coord_arr[k];

#ifndef NO_QC
        if (data_bot == MISSING) continue;
        for (; idx_top < N; ++idx_top) {
            if (data_arr[idx_top] != MISSING) break;
        }
#endif
        ...
        if (((data_bot <= data_val) && (data_top >= data_val)) ||
            ...
            const float dist =
                std::abs((data_val - data_bot) / (data_top - data_bot));
            if (is_pres) {
                return std::pow(10.0f, lerp(std::log10(coord_bot),
                                            std::log10(coord_top), dist));
            } else {
                return lerp(coord_bot, coord_top, dist);
            }
        }
    }
    return MISSING;
