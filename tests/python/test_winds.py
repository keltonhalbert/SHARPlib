...
def test_mean_wind():
    ...
    assert (mw.u == pytest.approx(4.474552, abs=1e-5))
    assert (mw.v == pytest.approx(12.094197, abs=1e-5))

    mw = winds.mean_wind(
        plyr, snd_data["pres"], snd_data["uwin"], snd_data["vwin"], True)

    assert (mw.u == pytest.approx(3.787980, abs=1e-5))
    assert (mw.v == pytest.approx(11.847604, abs=1e-5))
